...
class UNetModelMulti(L.LightningModule):
    ...
    def compute_loss(self, out, y):
        ...
        for i in range(12):
            loss += sigmoid_focal_loss(
                out[:, 3 * i], y[:, 3 * i], alpha=0.874, reduction="mean"
            )
            loss += sigmoid_focal_loss(
                out[:, 3 * i + 1], y[:, 3 * i + 1], alpha=0.983, reduction="mean"
            )
            loss += sigmoid_focal_loss(
                out[:, 3 * i + 2], y[:, 3 * i + 2], alpha=0.99, reduction="mean"
            )
        return loss

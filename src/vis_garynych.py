...
from src.datasets import (FullFeatRegSegDataLoader, FullFeatRegSegDataset,
                          IntensityRegSegDataset, ProcessedRadarDataset)
...
def prepare_data_loaders(train_batch_size=1, valid_batch_size=1, test_batch_size=1):
    valid_dataset = FullFeatRegSegDataset(file_path="val_full.hdf5", target_source=True)
...
def stack_res(bin_seg, reg, channels=3, to_abs=False):
    ...
    if to_abs:
        scale = torch.Tensor([0.5, 1, 3.5]).view(1, -1, 1, 1)
...
def evaluate_on_val(model, valid_loader):
    outdir = "./plt_vis/garrynych" + str(datetime.now())
    os.makedirs(outdir, exist_ok=True)
    iter_dt = iter(valid_loader)
    print(len(valid_loader))
    for i in range(4000):
        (velocity, intensity, reflectivity, events), target = next(iter_dt)
...
def main():
    ...
    checkpoint = "checkpoint/2023-11-12 00:35:52.929493/unet_garynych_epoch=38_val_loss=0.134067.ckpt"

...
class Merger:
    ...
    def append(self, x):
        ...
        # if self.type == 'sparse_mean':
        #     x = self._prepare_for_sparse_mean(x)

        if self.output is None:
            self.output = []
            self.output.append(x[0][None, ...])
            self.output.append(x[1][None, ...])
        ...
        elif self.type in ["median", "sparse_mean"]:
            self.output[0] = torch.cat([self.output[0], x[0][None, ...]])
            self.output[1] = torch.cat([self.output[1], x[1][None, ...]])
    ...
    def result(self):
        ...
        elif self.type == "sparse_mean":
            result = []
            result.append(self.output[0].nanmean(dim=0))
            result.append(self.output[1].nanmean(dim=0))
            result = tuple(result)
        elif self.type == "median":
            result = self.output.nanmedian(dim=0).values
        else:
            raise ValueError("Not correct merge type `{}`.".format(self.type))
        return result

    def _prepare_for_sparse_mean(self, x):
        mask = x != -1
        x[~mask] = 0
        return torch.stack([x, mask.float()])
